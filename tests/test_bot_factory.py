...
def test_create_bots_supports_heuristic_v3() -> None:
    bots = create_bots(("heuristic_v3", "heuristic_v3", "heuristic_v3", "heuristic_v3"))
    assert set(bots.keys()) == set(PLAYER_IDS)
    assert all(isinstance(bots[player_id], HeuristicBotV3) for player_id in PLAYER_IDS)
