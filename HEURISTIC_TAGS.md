...
| `v3_avoid_owned_suit_lead_when_live_alternative` | If a suit is fully owned and some other legal lead is still live, treat the owned-suit lead as a near-hard veto. This is suppressed during active moon defense. | `-6.0` | Everyone else is void in diamonds, but you can still lead a heart or spade that might lose the lead. | `avoid_owned_suit_lead` |
...
| `v3_safe_qs_cash_lead` | If `QS` is the floor spade and higher outside spades still exist, cash it safely now instead of treating it like a generic dangerous queen lead. | `+8.5` | All lower spades are already gone, only `KS` and/or `AS` remain outside, and leading `QS` guarantees it gets covered safely. | `safe_qs_cashout` |
